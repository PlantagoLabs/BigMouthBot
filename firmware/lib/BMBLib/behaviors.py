...
class MultiBehavior(AbstractBehavior):
    """Behavior that can switch between behaviors it is provide, based on name."""
...
class GoToPointBehavior(AbstractBehavior):
    ...
    def _make_position_vector(self, data):
        return [data['x'], data['y']]

    def start(self):
        self.start_point = self._make_position_vector(synapse.recall_message('estimate.position'))
    ...
    def play(self):
        current_position = self._make_position_vector(synapse.recall_message('estimate.position'))
        ...
        diff_heading = goal_heading - synapse.recall_message('estimate.position')['heading']
...
class GoToHeadingBehavior(AbstractBehavior):
    ...
    def play(self):
        diff_heading = self.target_heading - synapse.recall_message('estimate.position')['heading']
        print(self.target_heading, synapse.recall_message('estimate.position')['heading'], diff_heading)
        while diff_heading > math.pi:
            diff_heading -= 2*math.pi
        while diff_heading < -math.pi:
            diff_heading += 2*math.pi
